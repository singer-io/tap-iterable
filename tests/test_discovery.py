...
class IterableDiscoveryTest(IterableBase):
    """
    Test to verify that the discover mode and metadata conforms to standards.
    """

    def name(self):
        return "tap_tester_iterable_discovery_test"
    ...
    def test_run(self):
        ...
        for stream in streams_to_test:
            with self.subTest(stream=stream):
                ...
                actual_replication_keys = {item for item
                                           in set(stream_properties[0].get("metadata").get(self.REPLICATION_KEYS, []))
                                           if item}
